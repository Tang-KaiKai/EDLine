...
public:
    ...
    SobelOperator() = delete;

    SobelOperator ( int width, int height ) :
...
void SobelOperator::Compute ( const uchar* pImg )
{
#if defined AVX2
    Compute_AVX ( pImg );
    return;
...
#ifdef AVX2
void SobelOperator::Compute_AVX ( const uchar* pImg )
{
...
#ifdef SSE2
void SobelOperator::Compute_SSE ( const uchar* pImg )
{
...
#ifdef OpenCV
void SobelOperator::Compute_OpenCV ( const uchar* pImg )
{
...
#ifdef Common
void SobelOperator::Compute_Common ( const uchar* pImg )
{
    const int widthBytesShort = ImgWidth*sizeof ( short );
